...
contract Versus {
  ...
  function getPendingGameFields(uint256 pendingGameId) public view returns (uint256, uint256, address payable, bool){
  ...
  //In progress Games
  function getinProgressGameFields(address idIn) public view returns (uint256, address payable, string memory, address payable, string memory, bool){
  ...
  function endGame(address winnerIn) public payable {
    ...
    playerInfo[winner].balance+=inProgressGames[msg.sender].amount.mul(2);
    ...
    emit Result(inProgressGames[msg.sender].player1, inProgressGames[msg.sender].player2, winner, inProgressGames[msg.sender].amount.mul(2), block.timestamp);
  }
